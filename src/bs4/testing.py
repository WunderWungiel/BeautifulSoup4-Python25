...
<body>Goodbye.</body>
    ...
    def test_namespaced_html(self):
        ...
        """
        markup = """<ns1:foo>content</ns1:foo><ns1:foo/><ns2:foo/>"""
        soup = self.soup(markup)
    ...
    def test_python_specific_encodings_not_used_in_charset(self):
        ...
        for markup in [
            ...
            for encoding in PYTHON_SPECIFIC_ENCODINGS:
                ...
                assert 'meta charset=""' in encoded
...
class XMLTreeBuilderSmokeTest(object):
    ...
    def test_docstring_generated(self):
        soup = self.soup("<root/>")
        self.assertEqual(
            soup.encode(), '<?xml version="1.0" encoding="utf-8"?>\n<root/>')

    def test_xml_declaration(self):
        markup = """<?xml version="1.0" encoding="utf8"?>\n<foo/>"""
        soup = self.soup(markup)
        self.assertEqual(markup, soup.encode("utf8"))
    ...
    def test_python_specific_encodings_not_used_in_xml_declaration(self):
        ...
        for encoding in PYTHON_SPECIFIC_ENCODINGS:
            if encoding in (
                ...
                continue
            encoded = soup.encode(encoding)
            assert '<?xml version="1.0"?>' in encoded
            assert encoding.encode("ascii") not in encoded
...
"""
    ...
    def test_namespaces_are_preserved(self):
        ...
        self.assertEqual("http://example.net/", root['xmlns:'])
